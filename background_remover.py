...
def download_model(model_name: str = "mobilenetv3") -> Path:
    ...
    if model_name not in MODEL_URLS:
        raise ValueError(
            f"Unknown model: {model_name}. Choose 'mobilenetv3' or 'resnet50'"
        )
...
def load_rvm_model(
    model_name: str = "mobilenetv3", device: str = "auto"
) -> torch.nn.Module:
    """
    Load RVM model.
    ...
    try:
        model = torch.hub.load(
            "PeterL1n/RobustVideoMatting", model_name, trust_repo=True
        )
...
def apply_morphological_cleanup(
    ...
    logging.info(
        f"Applying morphological cleanup: erode={erode_kernel}, dilate={dilate_kernel}, median={median_size}, blur={blur_size}"
    )
...
def load_background_image(
    image_path: str, width: int, height: int, device: str = "cpu"
) -> torch.Tensor:
    """
    ...
    if orig_width != width or orig_height != height:
        bg_image = cv2.resize(
            bg_image, (width, height), interpolation=cv2.INTER_LANCZOS4
        )
    ...
    bg_tensor = (
        torch.from_numpy(bg_image_rgb).permute(2, 0, 1).float().div(255.0).to(device)
    )
...
def remove_background(
    ...
    if bg_color is None:  # Transparent
        ...
        if not output_path.endswith(".mov") and not output_path.endswith(".avi"):
            ...
            logging.info(
                f"Changed output to {output_path_temp} for alpha channel support"
            )
    else:
    ...
    if background_image:
        ...
        if bg_color is not None:
            ...
            bgr_tensor = (
                torch.tensor([bg_color[2], bg_color[1], bg_color[0]])
                .view(3, 1, 1)
                .to(device)
            )
    ...
    with torch.no_grad():
        while True:
            ...
            if frame_count % 30 == 0:
                logging.info(
                    f"Processed {frame_count}/{total_frames} frames ({frame_count * 100 // total_frames}%)"
                )
    ...
    temp_fd, temp_with_audio = tempfile.mkstemp(
        suffix=".mp4", prefix="bg_removed_audio_"
    )
    os.close(temp_fd)
    ...
    try:
        ...
        cmd = [
            ...
            output_path
            if bg_color is not None
            else output_path_temp,  # Video without audio
            "-i",
            input_path,  # Original video with audio
            "-map",
            "0:v:0",  # Take video from first input
            "-map",
            "1:a:0?",  # Take audio from second input (? makes it optional)
            "-c",
        ...
        if result.returncode == 0:
            ...
            logging.warning(
                f"Could not merge audio (video may have no audio track): {result.stderr}"
            )
            # Clean up temp file
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        print(
            "Usage: python background_remover.py input.mp4 output.mp4 [background_color]"
        )
