...
def segment_person_from_video(
    ...
    while True:
        ...
        if smooth_radius > 0:
            ...
            blurred_mask = cv2.GaussianBlur(
                mask_uint8, (smooth_radius * 2 + 1, smooth_radius * 2 + 1), 0
            )
        ...
        if bg_color is None and bg_image is None:
            ...
            output_rgb = (
                image_rgb * mask_3ch + bg_image[:, :, ::-1] * (1 - mask_3ch)
            ).astype(np.uint8)
            output_bgr = cv2.cvtColor(output_rgb, cv2.COLOR_RGB2BGR)
    ...
    try:
        cmd = [
            "ffmpeg",
            "-i",
            output_path_temp,  # Video without audio
            "-i",
            input_path,  # Original video with audio
            "-map",
            "0:v:0",  # Take video from first input
            "-map",
            "1:a:0?",  # Take audio from second input (optional)
            "-c",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        print(
            "Usage: python person_segmenter.py input.mp4 output.mp4 [background_color]"
        )
