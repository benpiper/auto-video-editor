...
def get_video_metadata(video_path: str) -> Tuple[float, Tuple[int, int]]:
    """Gets video duration and dimensions using FFprobe. Returns (duration_seconds, (width, height))."""
    import subprocess
    ...
    res_duration = subprocess.run(cmd_duration, capture_output=True, text=True, timeout=10)
    ...
    res_size = subprocess.run(cmd_size, capture_output=True, text=True, timeout=10)

    try:
        parts = res_size.stdout.strip().split('x')
        size = (int(parts[0]), int(parts[1]))
    except (IndexError, ValueError):
        logging.warning(f"Could not parse video dimensions from ffprobe output: {res_size.stdout}")
        size = (0, 0)
