...
def get_video_metadata(path: str) -> Tuple[float, Tuple[int, int]]:
    """Helper to fetch video duration and size directly via ffprobe to avoid MoviePy load times."""
    cmd = [
...
def extract_audio(video_path: str, audio_path: str):
    ...
    # 1. Fast check if audio exists using ffprobe
    check_cmd = [
        'ffprobe', '-v', 'quiet', '-print_format', 'json',
    ...
    try:
        ...
        # 2. Extract audio efficiently using ffmpeg
        extract_cmd = [
        ...
        logging.error(f"Error checking or extracting audio: {e}")
        return False
