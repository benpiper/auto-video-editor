...
def get_video_metadata(video_path: str) -> Tuple[float, Tuple[int, int]]:
    ...
    logging.info(f"Fetching metadata for {video_path}")
    probe_cmd = [
        "ffprobe",
        "-v",
        "error",
    ...
    try:
        ...
        duration_str = stream_data.get("duration", format_data.get("duration"))
        ...
        logging.info("Falling back to MoviePy for metadata extraction...")
...
def extract_audio(video_path: str, audio_path: str):
    ...
    try:
        ...
        logging.error(
            f"FFmpeg audio extraction failed: {e.stderr if hasattr(e, 'stderr') else e}"
        )
        return False
    except Exception as e:
...
def detect_silence(
    audio_path: str, min_silence_len: int = 2000, silence_thresh: int = -40
    ...
    silence_intervals_sec = [
        (start / 1000, end / 1000) for start, end in silence_intervals_ms
    ]
    ...
    if silence_intervals_sec:
        ...
        for i, (start, end) in enumerate(silence_intervals_sec, 1):
            duration = end - start
            logging.info(
                f"  Silence {i}: {format_timestamp(start)} - {format_timestamp(end)} (duration: {duration:.2f}s)"
            )
    else:
...
def get_encoding_params(
    use_gpu: bool, use_crf: bool, bitrate: str, crf: int, preset: str
):
    """
    ...
    try:
        ...
        if has_audio:
            if remove_silence:
                ...
                silence_intervals = detect_silence(
                    temp_audio_path, min_silence_len, silence_thresh
                )
            else:
        ...
        if has_audio:
            ...
            filler_intervals, transcript_text = detect_filler_words(
                temp_audio_path, filler_words_list=filler_words
            )
        else:
        ...
        try:
            ...
            if no_crossfade:
                ...
                if progress_callback:
                    progress_callback(
                        95, "Concatenating segments (this may take a while)..."
                    )
                concatenate_segments_ffmpeg(
                    segment_files,
                    output_path,
