    ...
    try:
        ...
        try:
            ...
            duration_cmd = [
                ...
                "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", working_video_path
            ]
            ...
            if size_result.stdout.strip():
                size_str = size_result.stdout.strip().split('\n')[0]
                original_size = list(map(int, size_str.split('x')))
        except Exception as e:
            logging.error(f"Error getting video metadata via ffprobe: {e}")
