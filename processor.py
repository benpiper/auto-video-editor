...
from core.vram_guard import vram_guard
from core.vram_info import get_available_vram_gb, select_whisper_model
import subprocess
import ffmpegcv
import numpy as np
...
def extract_sandbox_segment(
    input_path: str, output_path: str, duration: float = 30.0
) -> bool:
    """
    Extracts the first N seconds of video using efficient zero-copy stream cloning.
    Utilizes /dev/shm for high-speed scratch space.
    """
    logging.info(f"Extracting {duration}s sandbox segment from {input_path}...")
    try:
        # We use standard ffmpeg stream copy for true zero-copy performance.
        # This is compatible with ffmpegcv's philosophy of avoiding overhead.
        cmd = [
            "ffmpeg",
            "-y",
            "-ss",
        ...
        logging.info(f"Successfully extracted sandbox segment to {output_path}")
        return True
    except subprocess.CalledProcessError as e:
        logging.error(f"Failed to extract sandbox segment: {e.stderr}")
        return False
    except Exception as e:
        logging.error(f"Unexpected error during sandbox extraction: {e}")
        return False
...
def assemble_preview_video(
    ...
    """
    Assembles a preview video from keep intervals derived from removal_intervals.
    Uses ffmpegcv for high-speed frame-accurate rendering with NVENC support.
    """
    try:
        # 1. Filter and clip intervals to max_duration
        relevant_removals = []
        for start, end in removal_intervals:
            if start < max_duration:
                relevant_removals.append((start, min(end, max_duration)))

        # 2. Invert to get keep intervals
        keep_intervals = invert_intervals(relevant_removals, max_duration)

        if not keep_intervals:
            logging.warning("No keep intervals found for preview assembly.")
            return False

        # Open source video
        vid_in = ffmpegcv.VideoCapture(input_path)
        fps = vid_in.fps

        # Check for NVENC availability and select codec
        codec = "h264_nvenc"
        try:
            # We don't pass frame_size to constructor, it's inferred from first frame
            vid_out = ffmpegcv.VideoWriter(output_path, codec, fps)
        except Exception:
        ...
        total_frames_to_write = sum(
            int((end - start) * fps) for start, end in keep_intervals
        )
        frames_written = 0
        current_frame = 0
        ...
        for start, end in keep_intervals:
            ...
            for _ in range(num_frames):
                ...
                # Ensure frame is a contiguous numpy array
                if not frame.flags["C_CONTIGUOUS"]:
                    frame = np.ascontiguousarray(frame)

                try:
                    vid_out.write(frame)
                except (BrokenPipeError, Exception) as write_err:
                    if codec == "h264_nvenc":
                        logging.warning(
                            f"NVENC write failed ({write_err}), falling back to libx264..."
                        )
                        try:
                            vid_out.release()
                        except Exception:
                ...
                if progress_callback and frames_written % 30 == 0:
                    progress = min(frames_written / total_frames_to_write, 1.0)
                    progress_callback(
                        50 + int(progress * 50),
                        f"Assembling preview: {int(progress * 100)}%",
                    )

        vid_in.release()
        vid_out.release()
        logging.info(f"Successfully assembled preview video to {output_path}")
        return True

    except Exception as e:
        logging.error(f"Failed to assemble preview: {e}")
        # Try to get ffmpeg error if possible
        if (
            "vid_out" in locals()
            and hasattr(vid_out, "process")
            and vid_out.process.stderr
        ):
            try:
                err = vid_out.process.stderr.read().decode()
                if err:
                    logging.error(f"FFmpeg error: {err}")
            except Exception:
                pass
        import traceback
...
def extract_audio(
    video_path: str, audio_path: str, progress_callback: Optional[Callable] = None
):
    """Extracts audio from video file using PyAV. Returns True if audio exists, False otherwise."""
    return extract_audio_pyav(
        video_path, audio_path, progress_callback=progress_callback
    )
...
def detect_silence(
    ...
    """
    Detects silence in audio file with progress reporting.
    Args:
    ...
    """
    logging.info(
        f"Detecting silence (thresh: {silence_thresh}dB, min_len: {min_silence_len}ms)..."
    )
    audio = AudioSegment.from_file(audio_path)
    total_ms = len(audio)

    # For very short audio, just run it once
    if total_ms < 60000:
        if progress_callback:
            progress_callback(0.0, "Detecting silence...")
        intervals_ms = silence.detect_silence(
            audio, min_silence_len=min_silence_len, silence_thresh=silence_thresh
        )
        if progress_callback:
            progress_callback(1.0, "Silence detection complete")
    else:
        # For longer audio, process in 30s chunks to report progress
        chunk_size_ms = 30000
        intervals_ms = []
        # Use an overlap of min_silence_len to ensure we don't miss silences crossing chunk boundaries
        for start_ms in range(0, total_ms, chunk_size_ms):
            end_ms = min(start_ms + chunk_size_ms, total_ms)
            # We take a slightly larger chunk to avoid boundary issues, but handle overlap merge later
            chunk_end_ms = min(end_ms + min_silence_len, total_ms)
            chunk = audio[start_ms:chunk_end_ms]

            chunk_silences = silence.detect_silence(
                chunk, min_silence_len=min_silence_len, silence_thresh=silence_thresh
            )
            for s_start, s_end in chunk_silences:
                intervals_ms.append((start_ms + s_start, start_ms + s_end))

            if progress_callback:
                progress = min(end_ms / total_ms, 1.0)
                progress_callback(
                    progress, f"Detecting silence: {int(progress * 100)}%"
                )

    # Convert to seconds and merge overlapping segments from chunking
    if not intervals_ms:
        return []

    intervals_sec = [(start / 1000, end / 1000) for start, end in intervals_ms]
    return merge_intervals(intervals_sec, min_gap=0)


def detect_filler_words(
    audio_path: str,
    model_size: Optional[str] = None,
    filler_words_list: List[str] = None,
    redis_mgr=None,
) -> List[Tuple[float, float]]:
    """
    ...
    """
    return detect_filler_words_whisper(
        audio_path, model_size, filler_words_list, redis_mgr
    )


def detect_filler_words_whisper(
    audio_path: str,
    model_size: Optional[str] = None,
    filler_words_list: List[str] = None,
    redis_mgr=None,
    ...
    Returns:
        Tuple of (List of (start, end) tuples, transcript string).
    """
    # Auto-select model based on VRAM if not specified
    if not model_size:
        available_vram = get_available_vram_gb()
        model_size = select_whisper_model(available_vram)
        logging.info(
            f"Auto-selected Whisper model: {model_size} (Available VRAM: {available_vram:.1f}GB)"
        )

    # Optional VRAM Guard
    if redis_mgr:
        guard_context = vram_guard(redis_mgr)
    else:
    ...
    with guard_context:
        ...
        # Apply torch.compile to encoder for Regional optimization if supported
        try:
            if hasattr(torch, "compile"):
                logging.info("Applying torch.compile to Whisper encoder...")
    ...
    for segment in result["segments"]:
        for word in segment.get("words", []):
            ...
            if text in filler_words:
                ...
                logging.info(
                    f"    ✓ Filler word detected: '{text}' at {format_timestamp(start_time)} - {format_timestamp(end_time)} (duration: {duration:.2f}s)"
                )
...
def detect_freeze_frames(
    video_path: str, min_duration: float = 5.0, noise_tolerance: float = 0.001
) -> List[Tuple[float, float]]:
    """
    ...
    logging.info(
        f"Detecting freeze frames (min duration: {min_duration}s, noise tolerance: {noise_tolerance})..."
    )
    ...
    try:
        ...
        cmd = [
            ...
            video_path,
            "-vf",
            f"freezedetect=n={noise_tolerance}:d={min_duration}",
            "-f",
            "null",
            "-",
        ]
        ...
        if freeze_intervals:
            ...
            for i, (start, end) in enumerate(freeze_intervals, 1):
                duration = end - start
                logging.info(
                    f"  Freeze {i}: {format_timestamp(start)} - {format_timestamp(end)} (duration: {duration:.2f}s)"
                )
        else:
...
def merge_intervals(
    intervals: List[Tuple[float, float]], min_gap: float = 0.1
) -> List[Tuple[float, float]]:
...
def invert_intervals(
    intervals: List[Tuple[float, float]], total_duration: float
) -> List[Tuple[float, float]]:
    """
...
def extract_segments_ffmpeg(
    ...
    for i, (start, end) in enumerate(segments):
        ...
        logging.info(
            f"Extracting segment {i + 1}/{len(segments)}: {format_timestamp(start)} - {format_timestamp(end)}"
        )
        if progress_callback:
        ...
        try:
            ...
            cmd = [
                ...
                "-ss",
                str(start),  # Seek to start (accurate, frame-level)
                "-t",
                str(duration),  # Duration
                "-c:v",
                "libx264",  # Re-encode video for precise cuts
                "-preset",
                "medium",  # Balanced encoding preset for quality
                "-b:v",
                target_bitrate,  # Target bitrate (from source detection)
                "-maxrate",
                target_bitrate,  # Max bitrate
                "-bufsize",
                f"{int(target_bitrate[:-1]) * 2}M",  # Buffer size (2x bitrate)
                "-c:a",
                "aac",  # Re-encode audio
                "-b:a",
                "192k",  # Audio bitrate
                "-avoid_negative_ts",
                "1",  # Handle timestamp issues
                "-y",  # Overwrite output
                segment_file,
            ]
...
def concatenate_segments_ffmpeg(
...
def concatenate_segments_moviepy(
    segment_files: List[str],
    output_path: str,
    ...
    """
    Concatenate video segments using MoviePy to support smooth crossfades.
    """
    from moviepy.editor import VideoFileClip, concatenate_videoclips

    logging.info(
        f"Concatenating {len(segment_files)} segments with MoviePy (crossfade: {crossfade_duration}s)..."
    )
    clips = []
    try:
        for f in segment_files:
            clip = VideoFileClip(f)
            # Add subtle audio/video fades to prevent pops and ensure smooth transitions
            # Note: MoviePy handles padding for crossfades automatically with concatenate_videoclips
            if clip.audio:
                clip.audio = clip.audio.fx(lambda a: a.audio_fadein(0.05)).fx(
                    lambda a: a.audio_fadeout(0.05)
                )

            # Only crossfade if duration > 0
            if crossfade_duration > 0 and len(clips) > 0:
                clip = clip.crossfadein(crossfade_duration)

            clips.append(clip)

        # Concatenate with padding for crossfades
        final_clip = concatenate_videoclips(
            clips, padding=-crossfade_duration, method="compose"
        )
        ...
        write_args = {
            ...
            "temp_audiofile": os.path.join(
                os.path.dirname(output_path), f"temp_audio_{uuid.uuid4().hex[:8]}.m4a"
            ),
            "remove_temp": True,
        }
        if bitrate:
            write_args["bitrate"] = bitrate
        if preset:
            write_args["preset"] = preset
        if ffmpeg_params:
            write_args["ffmpeg_params"] = ffmpeg_params
    ...
    if use_gpu:
        ...
        try:
            ...
            if "h264_nvenc" not in result.stdout:
                logging.warning(
                    "NVENC encoder not found in FFmpeg. Falling back to CPU encoding."
                )
                logging.warning(
                    "To enable GPU encoding, install FFmpeg with NVENC support."
                )
                use_gpu = False
        except Exception as e:
            logging.warning(
                f"Could not check for NVENC: {e}. Falling back to CPU encoding."
            )
            use_gpu = False

    if use_gpu:
        logging.info(
            f"Using NVIDIA GPU (NVENC) for video encoding [Mode: {render_preset}]"
        )
        codec = "h264_nvenc"

        if render_preset == "speed":
            nvenc_preset = "hp"
            ffmpeg_params = [
                "-preset",
                "p1",
            ...
            ffmpeg_params = [
                ...
                bitrate,
                "-bufsize:v",
                f"{int(bitrate.rstrip('k')) * 2}k" if "k" in bitrate else "10M",
            ]
        else:
            nvenc_preset_map = {
                "veryslow": "hq",
        ...
        logging.info(f"Using CPU (x264) for video encoding [Mode: {render_preset}]")
        codec = "libx264"
...
def process_video(
    ...
    rvm_downsample: Optional[float] = None,
    use_segmentation: bool = False,
    seg_model: str = "general",
    ...
    rvm_blur: int = 0,
    render_preset: str = "speed",
    forced_remove_intervals: Optional[List[Tuple[float, float]]] = None,
    progress_callback: Optional[Callable[[int, str], None]] = None,
    redis_mgr=None,
):
    ...
    # Use RAMDisk if available for high-speed audio extraction
    ramdisk_path = "/dev/shm"
    if os.path.exists(ramdisk_path) and os.access(ramdisk_path, os.W_OK):
        temp_audio_path = os.path.join(
            ramdisk_path, f"temp_audio_{uuid.uuid4().hex[:8]}.wav"
        )
    else:
    ...
    try:
        if forced_remove_intervals is not None:
            logging.info(
                f"Bypassing detection: Using {len(forced_remove_intervals)} provided removal intervals"
            )
            merged_remove_intervals = forced_remove_intervals
            # Skip detections, but keep a placeholder
            silence_intervals = []
            filler_intervals = []
            freeze_intervals = []
            transcript_text = None
            ...
            def audio_progress_wrapper(p, msg):
                if progress_callback:
                    # Map 0.0-1.0 extraction progress to 5-10% range of the main job
                    main_p = 5 + int(p * 5)
                    progress_callback(main_p, msg)
            ...
            def silence_progress_wrapper(p, msg):
                if progress_callback:
                    # Map 0.0-1.0 to 10-20% range
                    main_p = 10 + int(p * 10)
                    progress_callback(main_p, msg)
            ...
            if has_audio:
                silence_intervals = detect_silence(
                    ...
                    min_silence_len,
                    silence_thresh,
                    progress_callback=silence_progress_wrapper,
                )
            else:
            ...
            if has_audio:
                if progress_callback:
                    progress_callback(
                        20, "Detecting filler words (this may take a while)..."
                    )
                filler_intervals, transcript_text = detect_filler_words(
                    temp_audio_path, filler_words_list=filler_words, redis_mgr=redis_mgr
                )
            else:
            ...
            if freeze_duration is not None and freeze_duration > 0:
                ...
                freeze_intervals = detect_freeze_frames(
                    working_video_path,
                    min_duration=freeze_duration,
                    noise_tolerance=freeze_noise,
                )

            # 5. Combine and Merge Intervals to Remove
            if progress_callback:
                progress_callback(50, "Calculating cuts...")
            all_remove_intervals = (
                silence_intervals + filler_intervals + freeze_intervals
            )
            logging.info(
                f"Total intervals to remove: {len(silence_intervals)} silence + {len(filler_intervals)} filler words + {len(freeze_intervals)} freeze frames = {len(all_remove_intervals)}"
            )

            merged_remove_intervals = merge_intervals(all_remove_intervals)
            logging.info(
                f"After merging overlapping intervals: {len(merged_remove_intervals)} removal segments"
            )
        ...
        if merged_remove_intervals:
            ...
            for i, (start, end) in enumerate(merged_remove_intervals, 1):
                ...
                logging.info(
                    f"  Segment {i}: {format_timestamp(start)} - {format_timestamp(end)} (duration: {duration:.2f}s)"
                )
            logging.info(
                f"Total duration to be removed: {format_timestamp(total_removed_duration)}"
            )
        ...
        logging.info(
            f"Original video dimensions: {original_size[0]}x{original_size[1]} (width x height)"
        )
        ...
        if merged_remove_intervals:
            ...
            logging.info(
                f"Final video duration: {format_timestamp(final_duration)} (removed {format_timestamp(total_duration - final_duration)})"
            )
        ...
        if len(keep_intervals) == 1 and keep_intervals[0] == (0.0, total_duration):
            ...
            if remove_background or use_segmentation:
                logging.info(
                    "Background removal is enabled - will process the video for background removal only..."
                )
                ...
                logging.info(
                    "The video is already optimized. Exiting without re-encoding."
                )
                video.close()
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                logging.info(
                    f"Detected source bitrate: {source_bitrate_mbps:.2f} Mbps, using {target_bitrate} for output"
                )
            else:
                target_bitrate = "4M"  # Default fallback
                logging.warning(
                    f"Could not detect source bitrate, using default {target_bitrate}"
                )
        except Exception as e:
            target_bitrate = "4M"  # Default fallback
            logging.warning(
                f"Error detecting bitrate: {e}, using default {target_bitrate}"
            )

        # 6. Extract segments and concatenate with FFmpeg (much faster than MoviePy)
        segment_files = []
        try:
            # Apply render_preset overrides
            if render_preset == "speed":
                logging.info(
                    "Applying 'SPEED' preset: forcing GPU encoding and hard cuts"
                )
                use_gpu_encoding = True
                no_crossfade = True
                # Ensure bitrate is reasonably high for single-pass
                if bitrate == "5000k":  # if still default
                    bitrate = "8000k"
                    logging.info(
                        "Speed mode: Bumped bitrate to 8000k for single-pass quality"
                    )
            elif render_preset == "quality":
                logging.info(
                    "Applying 'QUALITY' preset: enabling crossfades and high-fidelity encoding"
                )
                no_crossfade = False
                # Quality mode defaults to higher bitrate if not specified
                if bitrate == "5000k":
                    bitrate = "12000k"
                    logging.info(
                        "Quality mode: Bumped bitrate to 12000k for high fidelity"
                    )

            # Extract segments using FFmpeg with detected bitrate
            if progress_callback:
                progress_callback(60, "Extracting video segments...")
            segment_files = extract_segments_ffmpeg(
                working_video_path,
                keep_intervals,
                temp_dir=".",
            ...
            codec, preset_value, ffmpeg_params = get_encoding_params(
                use_gpu_encoding, use_crf, bitrate, crf, preset, render_preset
            )
            ...
            if no_crossfade:
                ...
                logging.info(
                    "Using FFmpeg stream copy (no re-encoding) for maximum speed"
                )
                if progress_callback:
                ...
                # Re-encode with crossfades using MoviePy (Quality/Professional mode)
                if progress_callback:
                    progress_callback(
                        95,
                        "Concatenating segments with crossfades (this will take longer)...",
                    )
                concatenate_segments_moviepy(
                    segment_files,
                    output_path,
            ...
            for segment_file in segment_files:
                if os.path.exists(segment_file):
                    try:
                        ...
                        logging.warning(
                            f"Failed to remove temporary segment {segment_file}: {e}"
                        )
        ...
        if (remove_background or use_segmentation) and os.path.exists(output_path):
            ...
            try:
                ...
                temp_fd, temp_bg_removed = tempfile.mkstemp(
                    suffix=".mp4", prefix="bg_removed_"
                )
                os.close(temp_fd)
        ...
        # Cleanup temporary audio and transcript
        for transient in [
            temp_audio_path,
            temp_audio_path.replace(".wav", "_transcript.txt"),
        ]:
        ...
        # Cleanup background removal temp file if it somehow leaked
        if "temp_bg_removed" in locals() and os.path.exists(temp_bg_removed):
            try:
                os.remove(temp_bg_removed)
            except Exception:
                pass
