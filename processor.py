...
def get_video_info(video_path: str) -> Tuple[Optional[float], Optional[Tuple[int, int]], bool]:
    """
    Get duration, dimensions, and audio presence using ffprobe.
    Replaces MoviePy's VideoFileClip for simple metadata fetching to avoid loading overhead.
    """
    import subprocess
    import json
    try:
        ...
        return duration, (width, height) if width and height else None, has_audio
...
def extract_audio(video_path: str, audio_path: str):
    ...
    _, _, has_audio = get_video_info(video_path)
    if not has_audio:
    ...
    try:
        ...
        if result.returncode != 0:
            logging.error(f"Failed to extract audio: {result.stderr}")
            return False
...
def process_video(input_path: str, output_path: str, min_silence_len: int = 2000, silence_thresh: int = -63, crossfade_duration: float = 0.2, bitrate: str = "5000k", crf: int = 18, preset: str = "medium", use_crf: bool = False, use_gpu_encoding: bool = False, no_crossfade: bool = False, filler_words: List[str] = None, freeze_duration: float = None, freeze_noise: float = 0.001, remove_background: bool = False, bg_color: str = "green", bg_image: Optional[str] = None, rvm_model: str = "mobilenetv3", rvm_downsample: Optional[float] = None, use_segmentation: bool = False, seg_model: str = "general", seg_threshold: float = 0.5, seg_smooth: int = 5, rvm_erode: int = 0, rvm_dilate: int = 0, rvm_median: int = 0, rvm_blur: int = 0, progress_callback: Optional[Callable[[int, str], None]] = None, remove_silence: bool = True):
    ...
    try:
        ...
        total_duration, original_size, _ = get_video_info(working_video_path)
        if total_duration is None:
            logging.error("Could not determine video duration. Aborting.")
            return {'status': 'error', 'error': 'Could not determine video duration'}
        ...
        if original_size:
            ...
            logging.warning("Could not determine original video dimensions.")
