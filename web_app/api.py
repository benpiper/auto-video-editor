...
def create_job():
    ...
    # 🛡️ SECURITY: Sanitize the filename to prevent path traversal
    ...
    # Handle background image - simplified for API: assume previously uploaded
    bg_image = data.get('bg_image')
    if bg_image:
        # 🛡️ SECURITY: Only allow files from the upload folder, no absolute paths
        bg_image = secure_filename(bg_image)
