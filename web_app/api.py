...
def create_job():
    ...
    # Sentinel 🛡️: Sanitize input filename to prevent path traversal
    ...
    # Basic params
    filename = data.get('original_filename', safe_input_filename)
    
    # Handle background image
    bg_image = data.get('bg_image')
    if bg_image:
        # Sentinel 🛡️: Prevent path traversal by only allowing files in the upload folder
        safe_bg_image = secure_filename(bg_image)
        bg_check = os.path.join(upload_folder, safe_bg_image)
